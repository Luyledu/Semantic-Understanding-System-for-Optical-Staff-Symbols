...
from utils.score import ScorePretreatmest,NotationParsing

C = ['Gclef','High_Gclef','DHigh_Gclef','Lower_Gclef','DLower_Gclef','Fclef','High_Fclef','DHigh_Fclef','Lower_Fclef','DLower_Fclef','Cclef','Soprano_Cclef','M_Soprano_Cclef','Tensor_Cclef','Baritone_Cclef']
L = ['Sharp', 'Flat', 'Natural','dot']
T = ['Natural','dot','Sharp', 'Flat', 'D_S', 'A_S', 'E_S', 'B_S', 'F_S', 'C_S', 'B_F', 'E_F', 'A_F','D_F', 'G_F', 'C_F',]
S = ['1_-10', '1_-9', '1_-8', '1_-7', '1_-6', '1_-5', '1_-4', '1_-3', '1_-2', '1_-1', '1_0', '1_1', '1_2', '1_3','1_4', '1_5', '1_6', '1_7', '1_8', '1_9', '1_10', '1_11', '1_12', '1_13', '1_14', '1_15', '1_16', '1_17','1_18', '1_19', '1_20',
...
R = ['Rests1', 'Rests2', 'Rests4', 'Rests8', 'Rests16','Rests32', 'Rests64', 'Rests128']
T_S = ['Sharp', 'D_S', 'A_S', 'E_S', 'B_S', 'F_S', 'C_S']
T_F = ['Flat', 'B_F', 'E_F', 'A_F','D_F', 'G_F', 'C_F']
...
def readXML(xml_path,class_obj):
    tree = ET.parse(xml_path)
    objects = []
    # for xml_name in tree.findall('filename'):
    #     objects.append(xml_name.text)
    for obj in tree.findall('object'):
        if obj.find('name').text in class_obj:
            obj_struct = {}
...
def draw(object,img_path,output_path):
    img = Image.open(img_path).convert('RGBA')
    img = ImageOps.exif_transpose(img)
    draw = ImageDraw.Draw(img)
    fontPath = "/System/Library/Fonts/Supplemental/Times New Roman Bold.ttf"
    font = ImageFont.truetype(fontPath, 30)
    font1 = ImageFont.truetype(fontPath, 25)
    for a in object:
        print(a)
        xmin = int(a['bbox'][0])
        ymin = int(a['bbox'][1])
        ...
        label = a['name']
        # if label == 'dot':
        #     # draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline='green', width=2)
        #     drawRect(img, (xmin, ymin, xmax, ymax), fill=(0, 110, 220, 100))

        if xmin == 745 or xmin == 744:
            continue

        else:
            # draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline='green', width=2)
            drawRect(img, (xmin, ymin, xmax, ymax), fill=(0, 110, 220, 100))
            draw.text((xmin, ymin - 35), label, fill=(0, 0, 0), font=font)
    ...
    for line in MN_E:
        for NOTE in line:
            ...
            drawRect(img, (int(xmin), int(ymin), int(xmax), int(ymax)), fill=(252, 125, 61, 100))
            draw.text((int(xmin), int(ymin) - 30), name, fill=(0, 0, 0), font=font)
...
if __name__ == '__main__':
    ...
    xml_path = '/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/yolov5-6.1/runs/detect/2009_detect/11_0/xml/1.xml'
    img_path = '/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/yolov5-6.1/test2009/11/1.jpg'
    out_path = '/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/yolov5-6.1/runs/detect/2009_detect/11_2/2.pdf'
    mung_filename_path = '/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/yolov5-6.1/runs/detect/2009_detect/11_0/csv/1.csv'
    objects = readXML(xml_path,L)
    draw(objects,img_path,out_path)