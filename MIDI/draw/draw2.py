...
L = ['Natural','dot']
large = ['D_S', 'A_S', 'E_S', 'B_S', 'F_S', 'C_S', 'B_F', 'E_F', 'A_F','D_F', 'G_F', 'C_F',]
T = ['Sharp', 'Flat', 'D_S', 'A_S', 'E_S', 'B_S', 'F_S', 'C_S', 'B_F', 'E_F', 'A_F','D_F', 'G_F', 'C_F',]
...
def visualise_gt(objects, img_dir):
    for id, img_path in enumerate(img_dir):
        ...
        for a in objects:
            ...
            ymax = int(a['bbox'][3])
            label = a['name']
            if label in C or label in large:
                draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline=(255, 0, 0), width=2)
                # draw.text((xmin, ymax), "large", fill=(255, 0, 0), font=font)
            elif label in small:
                draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline='green', width=2)
                # draw.text((xmin, ymin - 35), 'small', fill='green', font=font)
            # elif label == 'dot':
            #     draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline='green', width=2)
            ...
            #     draw.text((xmin, ymin - 18), label, fill='#00FF00', font=font)
            # elif label == 'Pno' or label == 'Wh' or label == 'Trb' or label == 'Sax' or label == 'Hrp' or label == 'Cb' or label=='Rec':
            #     draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline=(0, 0, 255), width=2)
            #     draw.text((xmin, ymin - 18), label, fill='#0000FF', font=font)
            ...
            #     draw.rectangle((xmin, ymin, xmax, ymax), fill=None, outline=((27,171,115)), width=2)
            #     draw.text((xmin, ymin - 18), label, fill='#1BAB73', font=font)
            ...
            #     draw.text((xmin, ymin - 18), label, fill='#1BAB73', font=font)
                ...
                # draw.text((xmin, ymax), 'medium', fill='blue', font=font1)
        img.save(img_path.replace('jpg','pdf'))

if __name__ == '__main__':

    fontPath = "/System/Library/Fonts/Supplemental/Times New Roman Bold.ttf" # 字体路径
    # root = '/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/Data/0'

    pic_path = r'/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/yolov5-6.1/aa'
    ann_path = '/Users/loufengbin/Documents/python/pythonProject/tensorflow/YOLO/yolov5-6.1/runs/detect/exp5/xml'
    font = ImageFont.truetype(fontPath, 30)
    font1 = ImageFont.truetype(fontPath, 25)

    xml_paths = os.listdir(ann_path)
    if '.DS_Store' in xml_paths:
        xml_paths.remove('.DS_Store')
